...
if choice == "Accueil":
    ...
    if date_debut > date_fin:
        ...
        if st.button("🔍 Chercher"):
            query = '''
                SELECT ch.IdChambre, ch.numero, ch.Etage, t.Type, h.Ville
                FROM Chambre ch
    ...
    if st.button("Exporter"):
        if dataset == "Clients":
            ...
            c.execute("""
                SELECT r.IdReservation, c.Nom, ch.numero, h.Ville, r.DateArrivee, r.DateDepart
                FROM Reservation r
                JOIN Client c ON r.IdClient = c.IdClient
                JOIN Chambre ch ON r.IdChambre = ch.IdChambre
                JOIN Hotel h ON ch.IdHotel = h.IdHotel
